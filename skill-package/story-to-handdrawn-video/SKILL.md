...
description: Convert Chinese story copy, article-derived drafts, ordered local images, or user-uploaded character IP references into a hand-drawn Remotion story video. Supports a locked user-approved colored-pencil diary default, a built-in 20-style library, custom character-reference images, the 海洋哥 IP profile, article-copy review gates, and a配音版后期流程 that defaults to synchronized 1.2x playback with no bottom subtitle stream. Use when the user asks to extract an article into video copy, review or diff that copy, use an uploaded character prototype, use the 海洋哥 IP, generate, import, restyle, preview, render, or produce a subtitle-free voiceover cut.
---
...
7. Produce a silent base MP4. Voiceover and optional BGM are post-production tasks.
8. When the user requests a配音版成片, run the repository voiceover postprocessor below; do not create an SRT/VTT unless the user explicitly asks for subtitles.
9. Report the scene count, duration, output path, speed, subtitle policy, and whether the result is plan-only, preview, or final.

## Voiceover finalization

The base renderer remains a silent H.264 picture track. For a配音版成片, use `scripts/postprocess_voiceover.py` after the base render:
...
- `--speed 1.2`: retime both video and audio together, preserving synchronization and voice pitch.
- `--subtitle-mode none`: explicitly map only the video and audio streams, so no subtitle/data stream is inherited or muxed.
- `--voice zh-CN-XiaoxiaoNeural --rate=-5%`: use the default Chinese narration voice unless the user chooses another installed TTS voice.

The helper also writes a standalone `.m4a` and a runtime JSON manifest. It keeps per-scene voice segments under the output directory for re-editing. Only when the user explicitly requests an external subtitle file, add `--subtitle-mode srt`; this writes an SRT beside the MP4 and still does not burn or mux it into the video.

Required post-production dependencies are a local `edge-tts` executable plus `ffmpeg` and `ffprobe`. If any is unavailable, stop with an actionable dependency error and do not claim that the narration was completed.
...
当前菜单包含彩铅日记、极简线条、儿童蜡笔、涂鸦信息图、圆珠笔、水墨、水彩、水粉绘本、Zine 拼贴、白板讲解和木刻社论等 20 种风格；完整 id、中文名、推荐题材和示例图以该 JSON 与 contact sheet 为准。
